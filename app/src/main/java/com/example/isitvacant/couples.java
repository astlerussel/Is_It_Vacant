...
public class couples extends AppCompatActivity {
    private RecyclerView couplesRecyclerList;
    EditText searchText;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        couplesRecyclerList = (RecyclerView) findViewById(R.id.couples_recycler);
        couplesRecyclerList.setLayoutManager(new LinearLayoutManager(this));
        searchText= findViewById(R.id.search);
        query = contactsRef.orderBy("name").whereEqualTo("category","couples");
        ...
        searchText.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                if (s.length()==0){
                    ...
                    query = contactsRef.orderBy("name").whereEqualTo("category","couples").startAt(s.toString()).endAt(s.toString() + "\uf8ff");
                    setUpRecyclerView(query);
                    adapter.setOnItemClickListener(new RestaurantsAdapter.OnItemClickListener() {
                        @Override
                        public void OnItemClick(DocumentSnapshot documentSnapshot, int position) {
    ...
    private void setUpRecyclerView(Query query) {
        ...
        RecyclerView recyclerView = findViewById(R.id.couples_recycler);
